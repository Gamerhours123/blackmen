...
local cfglocation = "rainycfg/" 
makefolder("rainycfg") 

--watermark
getgenv().values = {}
local watermark = loadstring(game:HttpGet'https://raw.githubusercontent.com/CatzCode/PikaHub/main/Libraries/UI/bruh.lua')()
watermark:CreateWatermark("Rainy.Solutions | version 1.02 | USER")
watermark.theme.accentcolor = Color3.fromRGB(233, 236, 73)
watermark.theme.accentcolor2 = Color3.fromRGB(221, 255, 0)
watermark.theme.topheight = 42
watermark.theme.TextColor = Color3.fromRGB(211, 195, 152)
...
function library:New(name) 
	...
	rainy.Name = "electric boogalo" 
	rainy.ResetOnSpawn = false 
	rainy.ZIndexBehavior = "Global" 
	rainy.DisplayOrder = 420133769 

	local UIScale = INST("UIScale") 
	UIScale.Parent = rainy 
	...
	Menu.Image = "http://www.roblox.com/asset/?id=7604907071" 
	...
	game:GetService("UserInputService").InputBegan:Connect(function(key) 
		if key.KeyCode == Enum.KeyCode.Insert then 
			rainy.Enabled = not rainy.Enabled 
			library.uiopen = rainy.Enabled 
		end 
	end) 
...
local Models = game:GetObjects("rbxassetid://7827057974")[1] 
repeat wait() until Models ~= nil 
local ChrModels = game:GetObjects("rbxassetid://7826053990")[1] 
...
for _,lua in pairs(listfiles("rainylua")) do 
	local luaname = GSUB(lua, "rainylua\\", "") 
	INSERT(allluas, luaname) 
end 

RunService.RenderStepped:Wait() 

local gui = library:New("rainy.solutions") 
...
luascripts:Element("Button", "load", {}, function() 
	loadstring(readfile("rainylua\\"..values.luas["lua scripts"].lua.Scroll))() 
end) 
...
settings:Element("Button", "Spectator List", nil, function()
...
antiaim:Element("Dropdown", "pitch", {options = {"Sus_down", "God", "wtf", "down", "sussy", "glitch","zero", "random", "fucker", "negative"}}) 
...
world:Element("Dropdown", "skybox", {options = {"none", "nebula", "vaporwave", "clouds","MC","Dababy","Rezero","Chill"}}, function(tbl) 
...
movement:Element("Button", "Velocity Graph", nil, function()
    local Players = game:GetService("Players")
    local RunService = game:GetService("RunService")
    local LocalPlayer = Players.LocalPlayer
    ...
    VelocityCounter.Color = Color3.new(255,255,255)
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "rainy.solutions" and "Join the winning team. Join rainy.solutions!" or values.misc.chat.type.Dropdown == "winning" and "$$$ Im Using rainy.solutions $$$" or values.misc.chat.type.Dropdown == "Discord" and "$$$ https://discord.gg/K6pN9RxB $$$" or values.misc.chat.type.Dropdown == "MOM" and "I FUCKED UR MOM I FUCKED UR MOM I FUCKED UR MOM" or values.misc.chat.type.Dropdown == "SEMI" and "🐀 RATTED BY SEMI  🐀 ❌ DONT FUCK ❌ WITH 👾BLOXSENSE 👾👨‍🦲USERS👨‍🦲", false, "Innocent", false, true) 
			wait(values.misc.chat["speed (ms)"].Slider/1000) 
			 
		end 
	end 
end) 
chat:Element("Dropdown", "type", {options = {"Winning", "rainy.solutions", "Discord", "MOM", "SEMI"}}) 
...
Frame.Position = UDim2.new(0,1500,0,25)
...
RunService.RenderStepped:Connect(function(step) 
	...
	if PlayerIsAlive then 
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then 
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "Sus_down" and -3  or values.rage.angles["pitch"].Dropdown == "wtf" and 99999999999999999999999999999999999999999999999999999999 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "negative" and -6 or values.rage.angles["pitch"].Dropdown == "glitch" and (0 + -5) or values.rage.angles["pitch"].Dropdown == "fucker" and -13 or values.rage.angles["pitch"].Dropdown == "sussy" and math.huge -1/0 -5 or values.rage.angles["pitch"].Dropdown == "zero" and 0 or values.rage.angles["pitch"].Dropdown == "God" and 15 or values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-10, 10)/10 or 2.5
			if values.rage.angles["extend pitch"].Toggle and (values.rage.angles["pitch"].Dropdown == "up" or values.rage.angles["pitch"].Dropdown == "down" or values.rage.angles["pitch"].Dropdown == "negative" or values.rage.angles["pitch"].Dropdown == "glitch" or values.rage.angles["pitch"].Dropdown == "Sus_down" or values.rage.angles["pitch"].Dropdown == "180" or values.rage.angles ["pitch"].Dropdown == "fucker") then 
				Pitch = (Pitch*2)/1.6 
			end 
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current) 
	...
	if values.misc.chat["kill say"].Toggle then 
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "#Winning with rainySense" and values.misc.chat["message"].Text or "Allah is the Man", false, "Innocent", false, true) 
	end 
end) 
...
print("Credit to Mexican Hook for TeamMate Damage GUI and Health script ")
print("Credit to Lunarsense for the most AAs")
print("Credit to RocketWare for velocity graph")
